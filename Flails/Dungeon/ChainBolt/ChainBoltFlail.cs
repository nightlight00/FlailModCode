...
namespace extraflails.Flails.Dungeon.ChainBolt
{
	class ChainBoltFlail : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Chained Bolt");
			Tooltip.SetDefault("While spinning, you build up a charge"
						    +"\nThe higher the charge, the more waterbolts are shot out");
		}
		...
		public override void SetDefaults()
		{
			...
			item.shoot = ModContent.ProjectileType<ChainBoltFlailProjSpin>();
			item.shootSpeed = 0;
	...
	class ChainBoltFlailProjSpin : ModProjectile
	{
		int ActualFlail = ModContent.ProjectileType<ChainBoltFlailProj>();
		...
		public override void AI()
		{
			...
			if (!player.channel)
			{
				...
				shootVel.Normalize();
				shootVel *= ShootSpeed;
				var waterboltdamage = projectile.damage * 2;
				if (chargeup > 70)
                {
					boltamount = 3;
                } else if (chargeup > 20)
				...
				projectile.position += Vector2.Normalize(new Vector2(shootVel.X, shootVel.Y)) * 45f;
				for (int i = 0; i < boltamount; i++)
				{
					Vector2 perturbedSpeed = new Vector2(shootVel.X, shootVel.Y).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (boltamount - 1))) * .5f; // Watch out for dividing by 0 if there is only 1 projectile.
					Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, ProjectileID.WaterBolt, waterboltdamage / 3, 0, projectile.owner);
				}
				if (boltamount == 1)
                {
				...
				Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootVel.X, shootVel.Y, ActualFlail, projectile.damage * 2, projectile.knockBack, projectile.owner, 0f, 0f);
			...
			if (chargeup == 20)
			{
				for (int i = 0; i < 20; i++)
				{
					var dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, 45, projectile.velocity.X * 0.4f, projectile.velocity.Y * 0.4f, 100, default, 1.5f);
					dust.noGravity = true;
					dust.velocity /= 2f;
				}
			}
	...
	class ChainBoltFlailProj : ModProjectile
	{
		private const string ChainTexturePath = "extraflails/Flails/Dungeon/ChainBolt/ChainBoltFlailChain";
		int ChainLength = 170; 
		int MaxChainLength = 280;
