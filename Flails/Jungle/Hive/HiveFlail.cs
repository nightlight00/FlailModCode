...
namespace extraflails.Flails.Jungle.Hive
{
	class HiveFlail : ModItem
	{

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("The Hive");
			Tooltip.SetDefault("Awaken my bees");
		}
		...
		public override void SetDefaults()
		{
			...
			item.shoot = ModContent.ProjectileType<HiveFlailProj>();
			item.shootSpeed = 16;
	...
	class HiveFlailProj : ModProjectile
	{
		float shootSpeed = 4;
		float shootCooldown = 20;

		private const string ChainTexturePath = "extraflails/Flails/Jungle/Hive/HiveFlailChain";

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("The Hive"); // Set the projectile name to Example Flail Ball
		}
		...
		public override void AI()
		{
			...
			if (projectile.ai[0] == 0f)
			{
				...
				float maxChainLength = 140f * playerget.ChainLength;
			...
			else if (projectile.ai[0] == 1f)
			{
				...
				float maxStretchLength = 260f * playerget.ChainLength; // This is the furthest the flail can stretch before being forced to retract. Make sure that this is a bit less than maxChainLength so you don't accidentally reach maxStretchLength on the initial throw.
			...
			if (shootCooldown == 0) {
				...
				Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, shootVel.X, shootVel.Y, beetype, (projectile.damage + beedamage) / 2, projectile.knockBack, Main.myPlayer, 0f, 0f);
				shootCooldown = 20;
			}
			shootCooldown -= 1;
		}
