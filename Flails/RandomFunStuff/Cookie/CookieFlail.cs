...
namespace extraflails.Flails.RandomFunStuff.Cookie
{
	class CookieFlail : ModItem
	{

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("The Cookie");
		}
		...
		public override void SetDefaults()
		{
			...
			item.shoot = ModContent.ProjectileType<CookieFlailProj>();
			item.shootSpeed = 18;
	...
	class CookieFlailProj : ModProjectile
	{
		float shootCooldown = 9;

		private const string ChainTexturePath = "extraflails/Flails/RandomFunStuff/Cookie/CookieFlailChain";

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Cookie Flail"); // Set the projectile name to Example Flail Ball
		}
		...
		public override void AI()
		{
			...
			if (projectile.ai[0] == 0f)
			{
				...
				float maxChainLength = 240f * playerget.ChainLength;
			...
			else if (projectile.ai[0] == 1f)
			{
				...
				float maxStretchLength = 380f * playerget.ChainLength; // This is the furthest the flail can stretch before being forced to retract. Make sure that this is a bit less than maxChainLength so you don't accidentally reach maxStretchLength on the initial throw.
			...
			{
				int cookieprojshot;
				int cookieprojdamage;
				int projamount = 1;
				if (Main.rand.NextBool(10))
                {
					cookieprojshot = ModContent.ProjectileType<CookieProj.Cursor>();
					cookieprojdamage = 1;
					projamount = 1;
                } else
                {
                    ...
                    {
                        cookieprojshot = ModContent.ProjectileType<CookieProj.CookieChunk1>();
						cookieprojdamage = 3;
						projamount = 1 + Main.rand.Next(2);
					}
                    else
                    {
						cookieprojshot = ModContent.ProjectileType<CookieProj.CookieChunk2>();
						cookieprojdamage = 3;
						Main.rand.Next(3);
					}
                }
				...
				for (int i = 0; i < projamount; i++) {
					...
					Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, cookieprojshot, projectile.damage / cookieprojdamage, projectile.knockBack, Main.myPlayer, 0f, 0f);
				}
				shootCooldown = 9;
			}
			shootCooldown -= 1;
