...
namespace extraflails.Flails.RandomFunStuff.TriangleFlail
{
    class TriangleFlail : ModItem
	{

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Magical Triangle");
			Tooltip.SetDefault("Everyones favorite");
		}
		...
		public override void SetDefaults()
		{
			...
			item.shoot = ModContent.ProjectileType<TriangleFlailProj>();
			item.shootSpeed = 15;
		...
		public override void AddRecipes()
		{
			...
			recipe.AddIngredient(ModContent.ItemType<WizardTriangle>());
	...
	class TriangleFlailProj : ModProjectile
    {
		float shootCooldown = 20;
		int type;

		private const string ChainTexturePath = "extraflails/Flails/RandomFunStuff/TriangleFlail/TriangleFlailChain";
		...
		public override void AI()
		{
			...
			if (projectile.ai[0] == 0f)
			{
				...
				float maxChainLength = 200f * playerget.ChainLength;
			...
			else if (projectile.ai[0] == 1f)
			{
				...
				float maxStretchLength = 320f * playerget.ChainLength; // This is the furthest the flail can stretch before being forced to retract. Make sure that this is a bit less than maxChainLength so you don't accidentally reach maxStretchLength on the initial throw.
			...
			if (shootCooldown == 0)
			{
				...
				//projectile.position += Vector2.Normalize(new Vector2(6, 6)) * 45f;
				for (int i = 0; i < numberProjectiles; i++)
				{
					float choices = 0 + Main.rand.Next(3);
					if (choices == 0)
                    {
					...
					Vector2 perturbedSpeed = new Vector2(6, 6).RotatedBy(MathHelper.Lerp(-rotation, rotation, i / (numberProjectiles - 1))) * .2f; // Watch out for dividing by 0 if there is only 1 projectile.
					Projectile.NewProjectile(projectile.position.X, projectile.position.Y, perturbedSpeed.X, perturbedSpeed.Y, type, projectile.damage / 2, 0f, player.whoAmI);
				}
				shootCooldown = 15;
			}
			shootCooldown -= 1;
    ...
    class WizardTriangle : ModItem 
	{
        public override void SetStaticDefaults()
        {
			DisplayName.SetDefault("Triangle");
			Tooltip.SetDefault("It has a nice ring to it");
        }
