...
namespace extraflails.Flails.Present
{
	class PresentFlail : ModItem
	{

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Bundle O' Joy");
		...
		public override void SetDefaults()
		{
			...
			item.shoot = ModContent.ProjectileType<PresentFlailProj>();
			item.shootSpeed = 17;
	...
	class PresentFlailProj : ModProjectile
	{
		float shootCooldown = 10;

		private const string ChainTexturePath = "extraflails/Flails/Present/PresentFlailChain";

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Present"); // Set the projectile name to Example Flail Ball
		}
		...
		public override void AI()
		{
			...
			if (projectile.ai[0] == 0f)
			{
				...
				float maxChainLength = 320f * playerget.ChainLength;
			...
			else if (projectile.ai[0] == 1f)
			{
				...
				float maxStretchLength = 370f * playerget.ChainLength; // This is the furthest the flail can stretch before being forced to retract. Make sure that this is a bit less than maxChainLength so you don't accidentally reach maxStretchLength on the initial throw.
			...
			if (shootCooldown == 0)
			{
				int numberProjectiles = 2 + Main.rand.Next(2); // 2 to 3 shots
				for (int i = 0; i < numberProjectiles; i++)
				{
					Vector2 shootVel = projectile.Center;
					Vector2 perturbedSpeed = new Vector2(1, 1).RotatedByRandom(MathHelper.ToRadians(360));
					...
					if (choice == 0)
                    {
						choice = ProjectileID.NorthPoleSnowflake;
                    } else if (choice == 1)
					{
						choice = ProjectileID.OrnamentFriendly;
					}
					Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, perturbedSpeed.X, perturbedSpeed.Y, choice, projectile.damage / 5, projectile.knockBack, Main.myPlayer, 0f, 0f);
				}
				shootCooldown = 10;
			}
			shootCooldown -= 1;
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			while (true)
			{
				...
				drawPosition += remainingVectorToPlayer * 40 / length;
